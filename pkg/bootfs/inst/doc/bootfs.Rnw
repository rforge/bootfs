...
The usage of the package is illustrated for three classification algorithms: PAMR (Prediction analysis for Microarrays, \cite{Tibshirani2002}), RF-Boruta (Random forests with the Boruta algorithm for feature selection, \cite{Kursa2010}) and SCAD-SVM (Support Vector Machines with Smoothly Clipped Absolute Deviation feature selection, \cite{Zhang2006}, implementation from \cite{Becker2011}). First of all load the package:
...
For illustration purpose, some data can be simulated:
...
The first step is to verify, if the feature selection algorithms perform sufficienctly on the given data set. For this, a crossvalidation of the different classification algorithms is run:
...
The above command uses the classification methods PAMR, SCAD-SVM and RF-Boruta and performs a 5-fold (ncv=5) crossvalidation on the training data, repeating the crossvalidation 2 times with different training/test set assignments.

The results are summarised as ROC curves, shown in figures \ref{fig:rocpam}, \ref{fig:rocrf} and \ref{fig:rocscad}.

\begin{figure}[htp]
\centering
\includegraphics[width=8cm,height=8cm]{rocpam}
\caption{Receiver Operator Characteristic (ROC) curve for the PAM algorithm cross-validation.}
\label{fig:rocpam}
\end{figure}
...
\caption{Receiver Operator Characteristic (ROC) curve for the random forest RF-Boruta algorithm cross-validation.}
\label{fig:rocrf}
\end{figure}
...
\caption{Receiver Operator Characteristic (ROC) curve for the SCAD-SVM algorithm cross-validation.}
\label{fig:rocscad}
\end{figure}
...
This might be useful to inspect the drawn features for each method separately. The parameter \emph{prob=.9} can be used to set the cutoff, how often a feature must cooccur with another feature, such that an edge is drawn between them. However, the general ranking of the importance of the features is done by generating the combined importance graph:
...
There are several arguments which customise the look of the importance graph. In this call, the \emph{vlabel.cex} argument defines the magnification factor of the node labels (each node corresponds to one feature). The argument \emph{filter} can be used to specify, how often a feature must cooccur with another, such that an edge is drawn between the two features. Figure \ref{fig:impgraph1} shows the result of the above call.

\begin{figure}[htp]
\centering
\includegraphics[width=12cm,height=12cm]{impgraph1}
\caption{Importance graph generated by \emph{resultBS}.}
\label{fig:impgraph1}
\end{figure}
