...
The usage of the package is illustrated for three classification algorithms: PAMR (Prediction analysis for Microarrays), RF-Boruta (Random forests with the Boruta algorithm for feature selection) and SCAD-SVM (Support Vector Machines with Smoothly Clipped Absolute Deviation feature selection). First of all load the package:
